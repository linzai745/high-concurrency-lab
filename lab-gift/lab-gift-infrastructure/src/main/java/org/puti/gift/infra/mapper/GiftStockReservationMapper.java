...
public interface GiftStockReservationMapper extends BaseMapper<GiftStockReservationDO> {
    ...
    @Select("""
                ...
                WHERE sync_batch_no = #{batchNo}
                  ...
                  AND sync_status = 20
            """)
    Integer countSyncingByBatchNo(@Param("batchNo") String batchNo);
    ...
    @Select("""
                SELECT DISTINCT sync_batch_no
                FROM gift_stock_reservation
                WHERE sync_status = 20
                  AND sync_batch_no IS NOT NULL
                  AND update_time < #{beforeTime}
                ORDER BY sync_batch_no ASC
                LIMIT #{limit}
            """)
    List<String> selectTimeoutSyncingBatchNos(
            @Param("beforeTime") LocalDateTime beforeTime,
            @Param("limit") int limit
    );
    ...
    @Update("""
                ...
                WHERE sync_batch_no = #{batchNo}
                  ...
                  AND sync_status = 20
            """)
    int resetSyncingToPendingByBatchNo(@Param("batchNo") String batchNo);
    ...
    @Update("""
                ...
                WHERE sync_batch_no = #{batchNo}
                  ...
                  AND sync_status = 20
            """)
    int recoverSyncedByFinishedBatchNo(@Param("batchNo") String batchNo);
}
